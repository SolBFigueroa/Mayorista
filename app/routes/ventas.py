...
def agregar_venta(venta : VentaCrear,  db: Session = Depends(get_db)):
...
def buscar_por_fechas_ventas(fecha_inicio: date, fecha_fin: date, db: Session = Depends(get_db)):
    ventas_db = db.query(Venta).filter(Venta.fecha >= fecha_inicio, Venta.fecha <= fecha_fin).all()
    return ventas_db

@router.get("/ventas/nombre/{name}/tickets/")
def buscar_ventas_por_producto(name: str, db: Session = Depends(get_db)):
...
def agregar_detalle(id_venta: int, nuevo_detalle : DetalleCrear, db: Session = Depends(get_db)):
...
def editar_detalle(detalle_id: int, cantidad_nueva : int, db: Session = Depends(get_db)):
    detalle_db = db.query(DetalleVenta).filter(DetalleVenta.id == detalle_id).first()
    if detalle_db is None:
        raise HTTPException(status_code=404, detail="Detalle no encontrado")
...
def reporte_por_periodo_ventas(fecha_inicio: date, fecha_fin: date, db: Session = Depends(get_db)):
...
@router.get("/ventas/reporte/productos/{fecha_inicio}/{fecha_fin}")
def reporte_productos_mas_vendidos(fecha_inicio: date, fecha_fin: date,db: Session = Depends(get_db)):
